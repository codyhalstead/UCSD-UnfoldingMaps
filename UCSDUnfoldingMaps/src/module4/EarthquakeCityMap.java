...
public class EarthquakeCityMap extends PApplet {
	...
	private void printQuakes() {
		int weqs = quakeMarkers.size();
		for (Marker i : countryMarkers){
			String countryName = i.getProperty("name").toString();
			//System.out.println( countryName + "Heeeeeeeeelp");
			int eqs = 0;
			for(Marker marker : quakeMarkers){
				EarthquakeMarker m = (EarthquakeMarker)marker;
				//System.out.println(m.isOnLand());
				if(m.isOnLand()){
					//System.out.println(m.getStringProperty("country") + "working");
					if(countryName.equals(m.getProperty("country").toString())){
						eqs++;
					}
				}
			}
			if(eqs > 0){
				weqs -= eqs;
				System.out.println(countryName + " : " + eqs);
			}
		}
		System.out.println("Ocean : " + weqs);
